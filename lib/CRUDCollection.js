...
var CRUDCollection = function(options){
  ...
  var outputList = function(req, res, list, key){
    var collection = res.collection(list, key);
    if (req.app.autoLink){
      collection = collection.linkEach('self', function(item, name){
        if (!!key){
          return req.uri.child(item[key]);  // allow 'options' to provide another key to link on.
        } else {
  ...
  this.handler = {
    GET : function(req, res){
      options.list(req, res, function(err, items, options){
        if (!!err){
          return res.status.internalServerError(err);
        }
        outputList(req, res, items, options);
      });
    }
  };
