      ...
      item.save(function (err) {
        if (!!err){
          ...
          mongoRes.postCreate(req, res, function(err){
            res.header('Location',  req.originalUrl + '/' + item._id);
            res.send('', 201);
          });
        }
      });
...
MongoResource.prototype.collectionGET = function(req, res){
  ...
  this.finder(req).execFind(function(err, docs){
    ...
    var items = _.map(docs, function(v, k){
      var links = {parent : {href : req.originalUrl}, self : {href: req.originalUrl + '/' + v.doc._id}}
      return JSON.parse(res.representer.individual(v.doc, links)); // TODO : fugly!!
    });
    res.show(items)
  });
}
