...
var JsonContextHelper = require('../index').JsonContextHelper;
var FetchContextHelper = require('../index').FetchContextHelper;
...
ContextFaker.prototype.route = function(module, done){
  ...
  var body = this._body;
  $.done = done;
  FetchContextHelper($, module, function(){
    module[$.req.method]($);
    $.req.end(body);
  });
};
...
var ContextFake = function(method, url, headers, body, expectations){
  ...
  this.router.getParentUrl = function(urlStr){
    return detour.Router.prototype.getParentUrl(urlStr);
  };
  this.done = function(){};
  this.uri = new UriContextHelper(this.router, url, protocol, hostname);
  ...
  JsonContextHelper(this);
  this.res.status = (new StatusManager()).createResponder(this.req, this.res);
  this.res.status.on('error', function(data){ /* do nothing */});
  this.res.status.emit = function(){};  // swallow events
  ...
  this.validate = function(){
    if (expectations.statusCode){
      assert.equal(this.res.statusCode,
