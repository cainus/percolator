...
Router.prototype.setRouteForNode = function(prefix_path, node){
    //console.log("PrefixPath: " + prefix_path);
    var router = this;
    var resourceName = node.path;
    var filePath = urlJoin(router.resourceDirectory, prefix_path, resourceName);
    ...
    // recurse!
    var newPrefixPath = urlJoin(prefix_path, node.path);
    //console.log("newPrefixPath: " + newPrefixPath);
    _.each(node.children, function(kid){
      router.setRouteForNode(newPrefixPath, kid);
    });
}
