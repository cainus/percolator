...
var JsonModule = function(options){
  ...
  if (!!options.upsert && (!!options.create || !!options.update)){
    throw "the options parameter should not have create() or update() if it has upsert().";
  }
...
// this takes a method handler like wildcard.GET and wraps it another function 
...
function setFetchedOnMethod(options, fn){
  ...
  return function($){
    options.fetch($, function(err, fetched){
      if (err === true){
