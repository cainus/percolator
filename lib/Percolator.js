...
Percolator = function(options){
  ...
  this.server.onRequest(function(handler, context, cb){
    context.req.app = that.options;
    var router = that.server.router;
    context.req.router = router;  // TODO is there any use-case for this?  delete?
    var req = context.req;
    req.uri = urlgrey(protocol + '://' + req.headers.host + req.url);
    var res = context.res;
    ObjectHelper(req, res);
    context.res.status = that.statusman.createResponder(req, res);
    that.onRequestHandler(handler, context, function(context){
      ...
      if (!that.mediaTypes.isAcceptable(accept)){
        ...
        that.defaultContextHandler(req, res, handler, cb);
      }
    });
  });
...
Percolator.prototype.defaultContextHandler = function(req, res, handler, cb){
  var that = this;
  req.pause();
